  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center">
      <section className="bg-white border-b-8 border-[#E14D4D] py-40 w-full relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-24 text-[20rem] font-black text-slate-50/50 leading-none group-hover:text-[#E14D4D]/5 transition-colors duration-1000 uppercase tracking-tighter">Value</div>
        <div className="container mx-auto px-6 relative z-10 text-center">
          <h1 className="text-6xl md:text-9xl font-black text-[#222222] uppercase tracking-tighter mb-4 scale-y-110">
            Our <span className="text-[#E14D4D]">Pricing</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
           {plans.map((plan, i) => (
             <div key={i} className="group relative">
                <div className={`bg-white border-[0.5px] border-slate-100 p-12 aspect-square flex flex-col items-center justify-center gap-10 hover:shadow-2xl transition-all duration-700 bg-grid-slate-200/20 group-hover:-translate-y-4 ${plan.title === 'Pro' ? 'bg-slate-100/50 shadow-2xl scale-105' : ''}`}>
                   <div className="flex flex-col items-center gap-2">
                      <h4 className="text-4xl font-black text-[#222222] uppercase tracking-widest">{plan.title}</h4>
                      <p className="text-2xl font-black text-[#E14D4D] tracking-widest leading-none mt-2">{plan.price}</p>
                   </div>
                   ...
                   <div className="absolute top-4 right-6 text-[10px] font-black text-slate-200 uppercase tracking-widest group-hover:text-[#E14D4D] transition-colors">{plan.title === 'Pro' ? 'BEST VALUE' : ''}</div>
                </div>
             </div>
           ))}
        </div>
