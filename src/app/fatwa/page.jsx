...
const FatwaPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-[var(--secondary)] text-foreground py-40 relative overflow-hidden">
        <div className="container mx-auto px-6 relative z-10">
          <div className="max-w-5xl mx-auto text-center flex flex-col items-center">
            ...
            >
              Fatwa <span className="text-[var(--primary)]">Corner</span>
            </motion.h1>
            <motion.p 
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center w-full max-w-4xl">
                <div className="relative flex-grow w-full group">
                    <Search className="absolute left-8 top-1/2 -translate-y-1/2 w-6 h-6 text-slate-500 group-focus-within:text-[var(--primary)] transition-colors" />
                    <input 
                        type="text" 
                        placeholder="Search the Fatwa archive (e.g. Finance, Marriage, Worship)..."
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full bg-background/5 border-2 border-white/10 rounded-[2rem] py-8 pl-20 pr-10 text-lg outline-none focus:border-[var(--primary)] focus:bg-background/10 transition-all font-bold placeholder:text-slate-600"
                    />
                </div>
                <button className="w-full md:w-auto bg-[var(--primary)] text-foreground px-16 py-8 rounded-[2rem] font-black uppercase tracking-widest text-xs hover:scale-105 active:scale-95 transition-all shadow-[0_25px_50px_-12px_rgba(225,77,77,0.4)]">
                    Ask a Question
                </button>
            </div>
            
            <div className="mt-16 flex items-center gap-12 text-slate-500 uppercase font-black text-[10px] tracking-[0.4em]">
                <span className="flex items-center gap-2"><Globe className="w-4 h-4 text-[var(--primary)]"/> Global Council</span>
                <span className="flex items-center gap-2"><ShieldCheck className="w-4 h-4 text-[var(--primary)]"/> Peer Reviewed</span>
                <span className="flex items-center gap-2"><BookOpen className="w-4 h-4 text-[var(--primary)]"/> Shariah Compliant</span>
            </div>
          </div>
        </div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1000px] h-[1000px] bg-[var(--primary)]/5 rounded-full blur-[150px] pointer-events-none"></div>
        <div className="absolute top-0 right-0 p-24 text-[25rem] font-black text-foreground/5 opacity-[0.03] leading-none select-none pointer-events-none translate-x-1/2">
             GUIDANCE
        </div>
      </section>

      {/* Trust & Methodology markers */}
      <section className="bg-slate-50 py-16 border-b border-border">
            ...
            <div className="flex flex-wrap justify-center gap-16 md:gap-32">
                {[
                    ...
                    <div key={i} className="text-center">
                        <h4 className="text-4xl font-black text-[var(--secondary)] tracking-tighter mb-1">{s.val}</h4>
      ...
      <section className="container mx-auto px-6 py-32">
        <div className="flex flex-col lg:flex-row gap-24">
            ...
            <aside className="lg:w-1/4 space-y-16">
                <div>
                   <h3 className="text-xs font-black text-[var(--secondary)] uppercase tracking-[0.4em] mb-10 border-l-4 border-[var(--primary)] pl-6">Browse by Subject</h3>
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`flex items-center justify-between px-8 py-6 rounded-3xl font-black uppercase tracking-widest text-[10px] transition-all duration-500 border-2 ${
                                    activeCategory === cat 
                                    ? 'bg-[var(--secondary)] border-[var(--secondary)] text-foreground shadow-2xl scale-105' 
                                    : 'bg-background border-slate-50 text-slate-400 hover:border-[var(--primary)] hover:text-[var(--primary)]'
                                }`}
                            >
                                {cat}
                                {activeCategory === cat && <motion.div layoutId="dot" className="w-2 h-2 rounded-full bg-[var(--primary)]" />}
                            </button>
                        ))}
                    </div>
                </div>

                <div className="bg-[var(--secondary)] p-12 rounded-[4rem] text-foreground relative overflow-hidden group">
                    <div className="relative z-10 space-y-8">
                        <Scale className="w-16 h-16 text-[var(--primary)] group-hover:rotate-12 transition-transform duration-700" />
                    ...
                    <div className="absolute -bottom-10 -right-10 text-[10rem] font-black text-foreground/5 pointer-events-none select-none leading-none rotate-12">فقه</div>
                </div>
            </aside>
            ...
            <div className="lg:w-3/4">
                <div className="flex items-center justify-between mb-16 border-b-2 border-slate-50 pb-12">
                    <h2 className="text-4xl font-black text-[var(--secondary)] uppercase tracking-tighter">Verified <span className="text-[var(--primary)]">Rulings</span></h2>
                    <div className="flex items-center gap-4">
                        <button className="flex items-center gap-3 px-6 py-3 bg-slate-50 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-[var(--secondary)] transition-colors">
                ...
                <div className="grid grid-cols-1 gap-12">
                        ...
                        {filteredFatwas.map((fatwa) => (
                            ...
                            >
                                <motion.div
                                    ...
                                    className="group relative bg-background p-12 rounded-[4rem] border-2 border-slate-50 hover:border-[var(--primary)]/20 hover:shadow-[0_60px_100px_-40px_rgba(34,34,34,0.1)] transition-all duration-700 cursor-pointer overflow-hidden"
                                >
                                    <div className="flex flex-col md:flex-row gap-12">
                                        <div className="flex-shrink-0">
                                            <div className="w-24 h-24 rounded-[2.5rem] bg-[var(--secondary)] flex items-center justify-center text-foreground font-black text-3xl group-hover:bg-[var(--primary)] transition-all duration-700 shadow-xl group-hover:shadow-[var(--primary)]/30 group-hover:-rotate-6">
                                                Q
                                            </div>
                                        </div>
                                        <div className="flex-grow space-y-8">
                                            <div className="flex flex-wrap items-center gap-6">
                                                <span className="text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full bg-slate-50 text-slate-500 border border-border">{fatwa.category}</span>
                                            ...
                                            <h3 className="text-3xl font-black text-[var(--secondary)] leading-[1.1] uppercase tracking-tighter group-hover:text-[var(--primary)] transition-colors mb-6">
                                                {fatwa.question}
                                            </h3>

                                            <div className="relative pl-12 border-l-4 border-slate-50 group-hover:border-[var(--primary)]/20 transition-all">
                                            ...
                                            <div className="pt-10 border-t border-slate-50 flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-[9px] font-black uppercase tracking-widest text-[var(--secondary)]">{fatwa.mufti}</p>
                                                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-300">Council Mufti</p>
                                                    </div>
                                                </div>
                                                <button className="flex items-center gap-4 text-[11px] font-black uppercase tracking-[0.3em] text-[var(--secondary)] group-hover:text-[var(--primary)] transition-all group-hover:gap-6">
                    ...
                    {filteredFatwas.length === 0 && (
                        <div className="py-40 text-center flex flex-col items-center">
                            ...
                            <h3 className="text-3xl font-black text-[var(--secondary)] uppercase tracking-tighter mb-4">No Rulings Found</h3>
                            ...
                            <button 
                                ...
                                className="text-[var(--primary)] font-black uppercase tracking-[0.3em] text-[11px] border-b-2 border-[var(--primary)] pb-1 hover:pb-2 transition-all"
                            >
